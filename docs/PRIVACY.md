...
- specialty/research profile answers, navigation overrides, taxonomy assignments, evidence,
...
The taxonomy catalog is bundled and validated locally; it is not fetched by scraping a
credential or terminology site at runtime. No external terminology service is enabled in
Phase 1. Future terminology mappings require explicit provenance, version, licensing, and
network-consent review.
...
beta does not automatically transmit a support bundle. Profile answers, taxonomy
assignments, assignment evidence, and disease-state values are also excluded.
...
The bundled taxonomy is a literature-organization starter catalog, not a complete clinical
ontology. Automatic disease-state extraction is disabled and remains experimental until
field-specific evidence and accuracy gates pass.
